...
def initialize_database():
    with dbapi2.connect(app.config['dsn']) as connection:
        ...
        query = """CREATE TABLE STUDENTBRANCHES_CASTING(
                    STUDENTBRANCH_ID INTEGER REFERENCES STUDENTBRANCHES(ID) ON DELETE CASCADE,
        ...
        query = """CREATE TABLE BRANCHFEEDS (
                ID SERIAL PRIMARY KEY,
                BRANCH_ID INTEGER REFERENCES STUDENTBRANCHES(ID) ON DELETE CASCADE,
                USER_NAME VARCHAR(20) REFERENCES USERS(USERNAME) ON DELETE CASCADE,
